...
class ConcurrencyGate:
    """Fail-fast gate: reject with 429 when at capacity (no queue).

    Check-and-increment happens atomically under one lock, so concurrent
    acquires cannot slip past the limit (no check-then-acquire race).
    """
